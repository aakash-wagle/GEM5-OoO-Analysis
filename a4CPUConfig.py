...
def createCPU(options, processes, system):
    ...
    if len(options.cse530_core_config.split("_")) == 3:
        core_configs = options.cse530_core_config.split("_")
        branch_predictor = core_configs[0]
        num_robs = int(core_configs[1])
        issue_width = int(core_configs[2])
        options.cse530_issue_width = issue_width
        options.branch_predictor = branch_predictor
        options.cse530_num_robs = num_robs
